function [fref,gref] = Helmholtz_n_tau(solver,G,tau,MagRs,ArgRs,MagRn,ArgRn,alpha_damp,configs,s_init)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Helmholtz solver
%
% Inputs:
...
% Ref: [1] C. Silva, L. Magri, T. Runte, W. Polifke, Uncertainty
% quantification of growth rates of thermoacoustic instability by an
% adjoint Helmholtz solver, J. Eng. Gas Turbines and Power 139 (2017)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     flame.gamma = 1.4; % [-]
%     flame.alpha = 0.3; % Relaxation Parameter

    % Set values for FDF FLAME B
      flame.uref = 4.16; % m/s
      flame.Qref = 3030; % W
      flame.rhoref = 1.177; % kg/m3
      flame.gamma = 1.4; % [-]
      flame.alpha = 0.7; % Relaxation Parameter
    ...
    switch solver
        ...
        case 'Secant'
            ...
            while 1 %abs(ds) > 1e-2 && iter < iterlim
                ...
                flame.FTF = G*exp(-svec(1)*tau);
                H1 = sflame(mesh, index, dim, l, flame, thin);
                flame.FTF = G*exp(-svec(2)*tau);
